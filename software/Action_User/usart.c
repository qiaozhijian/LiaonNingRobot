...
void PostionUSART3_Init(uint32_t BaudRate)
{
...
void CameraUSART2_Init(uint32_t BaudRate)//串口2
{
  GPIO_InitTypeDef 	GPIO_InitStructure;
	...
	//串口3对应引脚复用映射
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource5,GPIO_AF_USART2); //GPIOC5复用为USART2
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource6,GPIO_AF_USART2); //GPIOC6复用为USART2
...
//PC12:  UART5 Tx
//PD2 :  UART5 Rx
void TestUART5_Init(uint32_t BaudRate)
{
	NVIC_InitTypeDef NVIC_InitStructure;
	GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
